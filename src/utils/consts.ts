...
export const getSTART_MESSAGE_STEP_1 = (name?: string) => {
  return `
👋 ${name ? `Здравствуйте, ${name}\\!` : 'Здравствуйте\\!'}  
Я \\- Ваш универсальный ИИ\\-ассистент\\.
Я могу помочь с огромным количеством задач: от составления плана тренировок до перевода текстов на любой язык\\. Могу быть Вашим помощником, преподавателем, психологом и много кем еще\\.
...
Мой потенциал практически безграничен, вот малая часть того, что я могу:
🔄 перевести текст на любой язык
✍️ написать пост, статью, имеил или краткое изложение
📚 решить задачи и написать учебные работы
💡 придумать контент\\-план, историю, сценарий или любой другой креатив
...
*Всего есть 4 уровня подписки:*

→ *${SUBSCRIPTIONS.START.title} ${SUBSCRIPTIONS.START.icon}*
_20 базовых запросов в день и 10 генераций изображений в месяц_
→ *${SUBSCRIPTIONS.OPTIMUM.title} ${SUBSCRIPTIONS.OPTIMUM.icon}*
_50 базовых запросов в день, 50 PRO запросов и 20 генераций изображений в месяц_
→ *${SUBSCRIPTIONS.PREMIUM.title} ${SUBSCRIPTIONS.PREMIUM.icon}*
_100 базовых запросов в день, 100 PRO запросов и 30 генераций изображений в месяц_
→ *${SUBSCRIPTIONS.ULTRA.title} ${SUBSCRIPTIONS.ULTRA.icon}*
...
Подробнее почитать про токены и уровни вы можете по команде /help
`;
...
_Например:_
– Объясни, что такое _\\*любая тема\\*_ так, чтобы понял ребенок
...
– Посоветуй 5 сериалов, если мне понравились _\\*названия Ваших любимых сериалов\\*_
`;
