...
*Можете\\!*
У меня есть и бесплатная версия, и платная подписка с расширенными возможностями\\.
...
*Уровни подписки и цены \\(в месяц\\):*
...
Чтобы открыть его, нажмите на значок квадрата в правой части поля ввода сообщения \\(обведен красным кругом на фото\\)\\. При нажатии откроется меню с кнопками для удобного управления\\. Вы также можете пользоваться командами: для этого нажмите на три синюю кнопку с тремя полосками *слева* от поля ввода\\.
`;
...
export const getNoBalanceMessage = (
  ...
  const modelLabel = AiModelsLabels[model].replace(/-/g, '\\-');
...
→ Доступ к продвинутым моделям \\(начиная с уровня Оптимум\\)
...
💎 Кроме того, платные пользователи первыми получают доступ к новым функциям\\!
  ...
  return `
*⨯ Нет доступных запросов для обращения к ${modelLabel} ⨯*
Чтобы продолжать пользоваться моделью, ${
    isFreeUser
