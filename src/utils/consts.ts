...
- Никогда не используй знаки форматирования и экранирования, такие как *, _, <, > и т.д. Всегда вместо **Заголовок** или <Заголовок> используй Заголовок. Также не используй escape-последовательности в тексте, такие как \\* и \\<\\>, то есть вместо \(x\) пиши (x). Если формула содержит дробь, используй текстовый вид, например: "x = 1/2" вместо дробного представления.
– Никогда не используй форматирование формул или особую разметку или язык формул: например, latex. Вместо этого пиши формулы обычными простыми человекочитаемыми математическими символами. Всегда отдавай текст без специальных символов, используй только обычные текстовые обозначения. Все математические, физические или другие специализированные обозначения должны быть записаны словами или с использованием простых текстовых символов.
...
→ И *платная* – отвечает на голосовые сообщения, даёт доступ к PRO модели, генерации картинок и расширенной памяти в диалоге
...
→ Сделать запрос к PRO модели \\(смените модель на GPT\\-4o в /settings\\)
