...
Чтобы открыть меню, нажмите на значок квадрата в правой части поля ввода сообщения \\(обведен красным кругом на фото\\)\\. При нажатии откроется меню с кнопками для удобного управления\\. В также можете пользоваться командами: для этого нажмите на три синюю кнопку с тремя полосками *слева* от поля ввода\\.
  `;
